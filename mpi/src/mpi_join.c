...
int main(int argc, char *argv[]) {
  ...
  /* 3e: Alltoallv — use MPI_BYTE, scale counts/displs by sizeof(Trip) */
  Trip *recv_buf = malloc(total_recv * sizeof(Trip));

  int *send_byte_counts = malloc(numtasks * sizeof(int));
  int *recv_byte_counts = malloc(numtasks * sizeof(int));
  int *send_byte_displs = malloc(numtasks * sizeof(int));
  int *recv_byte_displs = malloc(numtasks * sizeof(int));

  for (int i = 0; i < numtasks; i++) {
    send_byte_counts[i] = send_counts[i] * (int)sizeof(Trip);
    recv_byte_counts[i] = recv_counts[i] * (int)sizeof(Trip);
    send_byte_displs[i] = send_displs[i] * (int)sizeof(Trip);
    recv_byte_displs[i] = recv_displs[i] * (int)sizeof(Trip);
  }

  MPI_Alltoallv(send_buf, send_byte_counts, send_byte_displs, MPI_BYTE,
                recv_buf, recv_byte_counts, recv_byte_displs, MPI_BYTE,
                MPI_COMM_WORLD);
  ...
  free(send_displs);
  free(recv_displs);
  free(send_byte_counts);
  free(recv_byte_counts);
